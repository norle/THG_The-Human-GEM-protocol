...
model1 = cobra.io.read_sbml_model(
    os.path.join(project_root, "models", "Human-GEM1_19.xml")
)
model12 = cobra.io.read_sbml_model(
    os.path.join(project_root, "models", "THG-beta1.xml")
)
model15 = cobra.io.read_sbml_model(
    os.path.join(project_root, "models", "THG-beta2.xml")
)
modelTHG = cobra.io.read_sbml_model(
    os.path.join(project_root, "models", "THG-2023-02-25.xml")
)
...
plt.savefig(
    os.path.join(
        current_dir, "figures", "metabolites_annot_group-THG-beta-1_vs_Human1.svg"
    ),
    format="svg",
)
...
plt.savefig(
    os.path.join(
        current_dir, "figures", "reaction_annot_group-THG-beta-1_vs_Human1.svg"
    ),
    format="svg",
)
...
l2 = [
    len(
        [
            ...
            if sorted([*x.annotation]) == ["bigg.metabolite", "sbo", "vmhmetabolite"]
        ]
    )
    ...
    * 100,
    len(
        [
            x
            for x in model12.metabolites
            if sorted([*x.annotation]) == ["bigg.metabolite", "sbo", "vmhmetabolite"]
        ]
    )
    / len(model12.metabolites)
    * 100,
    len(
        [
            x
            for x in model15.metabolites
            if sorted([*x.annotation]) == ["bigg.metabolite", "sbo", "vmhmetabolite"]
        ]
    )
    / len(model15.metabolites)
    * 100,
]
r = np.array(l2)
...
ax.legend(
    [z, y, x],
    ["THG-beta-2", "THG-beta-1", "Reference GEM"],
    fontsize=size,
    title="Model",
    title_fontsize=size,
)
...
plt.savefig(
    os.path.join(
        current_dir,
        "figures",
        "non_annotated_metabolites-THG-beta-1_vs_THG-beta-1_vs_Human1.svg",
    ),
    format="svg",
)
...
z = plt.barh(
    range(len(l)), memote[0:, 0], color="darkslateblue", tick_label=l
)  ##8390FA

ax.tick_params(axis="both", labelsize=size)
ax.legend(
    [h, x, y, z],
    ["THG", "THG-beta-2", "THG-beta-1", "Reference GEM"],
    fontsize=size,
    title="Model",
    title_fontsize=size,
)
...
plt.savefig(
    os.path.join(
        current_dir, "figures", "MEMOTE-THG_vs_THG-beta-1_vs_THG-beta-1_vs_Human1.svg"
    ),
    format="svg",
)
...
x = np.array(
    [
        len(model1.genes),
        len([x for x in model1.reactions if x.gene_reaction_rule]),
        len(model1.metabolites),
        len(model1.reactions),
    ]
)
y = np.array(
    [
        len(model12.genes),
        len([x for x in model12.reactions if x.gene_reaction_rule]),
        len(model12.metabolites),
        len(model12.reactions),
    ]
)
z = np.array(
    [
        len(model15.genes),
        len([x for x in model15.reactions if x.gene_reaction_rule]),
        len(model15.metabolites),
        len(model15.reactions),
    ]
)
h = np.array(
    [
        len(modelTHG.genes),
        len([x for x in modelTHG.reactions if x.gene_reaction_rule]),
        len(modelTHG.metabolites),
        len(modelTHG.reactions),
    ]
)
...
plt.savefig(
    os.path.join(
        current_dir,
        "figures",
        "model_components-THG_vs_THG-beta-1_vs_THG-beta-1_vs_Human1.svg",
    ),
    format="svg",
)
...
plt.savefig(
    os.path.join(
        current_dir,
        "figures",
        "isoenzyme_in_new_compartment_reactions-THG-beta-1_vs_THG-beta-2.svg",
    ),
    format="svg",
)
...
counts = [84.10, 15.90]
counts = pd.Series(counts, index=[str(counts[0]) + " RP", str(counts[1]) + " WP"])
